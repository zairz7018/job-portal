...
const TalentCard = (props: any) => {
  ...
  useEffect(() => {
    if (props.applicantId) {
      getProfile(props.applicantId)
        ...
        .catch((err) => console.log("[TalentCard] Error fetching profile:", err));
    } else {
      setProfile(props);
    }
  }, [props]);
  ...
  return (
    ...
    transition duration-300 ease-in-out bs-mx:w-[48%] md-mx:w-full 
    ">
      ...
      <div className="flex gap-2 flex-wrap">
        {profile?.skills?.map((skill: any, index: any) =>
          index < 4 ? (
            <div key={index} className="py-2 bg-mine-shaft-800 text-bright-sun-400 rounded-lg text-xs">
              {skill}
            </div>
          ) : null
        )}
