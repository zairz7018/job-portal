...
const JobDesc = (props:any) => {
  ...
  return <div className="w-2/3">
...

    {
      ...
      <div className="font-semibold" >{props?props[item.id]: "NA" } {item.id==="packageOffred " && <>Euro</>}</div>
    </div>
      )
    }
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
            {
              props?.skillsRequired?.map((item:any,index:number) => <ActionIcon key={index} color="brightSun.4" className="!h-fit !w-fit font-medium !text-sm" variant="light" p="xs" size="lg" radius="xl" aria-label="Settings" > {item}
